    ...
    <div class="content py-4">
        <div class="container-fluid">
            <div class="card">
                <div class="card-body">
                    <h1>{{ $title }}</h1>
                    <form action="{{ route('user_dash.cmExperts.update', $expert) }}" method="POST"
                        ...
                        <div class="mb-3">
                            ...
                            <select name="doctor_id" class="form-control @error('doctor_id') is-invalid @enderror">
                                @foreach ($doctors as $doctor)
                                    <option @selected($doctor->id == $expert->doctor_id) value="{{ $doctor->id }}">
                                        {{ $doctor->id . ' - ' . $doctor->name }}</option>
                                @endforeach
                            </select>
                            @error('doctor_id')
                                <small class="invalid-feedback">{{ $message }}</small>
                        ...
                        <div class="mb-3">
                            ...
                            <input id="hour_price" name="hour_price" type="text" placeholder="Name"
                                class="form-control @error('hour_price') is-invalid @enderror "
                                value="{{ old('hour_price', $expert->hour_price) }}" />
                            @error('hour_price')
                                <small class="invalid-feedback">{{ $message }}</small>
                            @enderror
                        </div>
