    ...
    <div class="content">
        <div class="container-fluid">
            <div class="card mt-4">
                <div class="card-body">
                    <h1>{{ $title }}</h1>
                    <form action="{{ route('user_dash.doctor.dash.availableTimeUpdate', $AvailableTime) }}" method="POST"
                        ...
                        <div class="mb-3">
                            ...
                            <select name="expert_id" class="form-control @error('expert_id') is-invalid @enderror">
                                @foreach ($experts as $expert)
                                    <option @selected($AvailableTime->expert_id == $expert->id) value="{{ $expert->id }}">
                                        {{ $expert->id . ' - ' . $expert->name }}</option>
                                @endforeach
                            </select>
                            @error('expert_id')
                                <small class="invalid-feedback">{{ $message }}</small>
